...
> 状态：已完成主界面入口、首款游戏前端、固定伴生作品的整包游戏卡、受控详情程序包、FYOW/3 评论协议、分页读取、地图增量同步、玩家本地私有数据、私信唤醒、真实时间规则引擎、玩家/将领增量战力修炼及将领世界书同步。宿主通讯已统一为带请求编号的 `fyow-host/1`，玩家行动串行执行，公开提交前统一回滚，公开提交后的模型奖励进入本地待重试队列。评论区只追加领地、驻军与部署将领档案；私人保管箱、行动事件、入场答案、本人战力进度和其他玩家位置均不上传。完整接口见 `docs/online-world-communication-interfaces.md`。
>
...
变更记录本身不携带可由客户端伪造的行动时间。宿主使用评论接口返回的 `created_at` 作为唯一公共顺序；时间戳完全相同才用评论 ID 做稳定次序。客户端对每个格子和已部署将领分别记录最后一次已应用顺序，因此迟到的旧评论不会覆盖新领地，无关格子的迟到评论仍能补齐。每条玩家地图记录还携带服务端公共状态中的 `playerEpoch`；服主重置玩家时世代号递增，旧世代评论永久失效。初始、迁移和压缩快照只包含公共格子、公开参与者通讯密钥、封禁状态、玩家世代号及已部署将领，不含其他玩家位置、加入时间、金币、队列或随行将领。在线服主每观察到 32 条地图增量会自动发布覆盖快照，任何玩家重置/封禁/解封后也立即发布；读取端只有在分页时间严格单调时才可在最新完整快照处停止，平台分页混排时须全页读取并按评论时间归并，不能把新控制记录误判为旧页废弃信息。
...
模型请求由当前玩家工具发起，结果先保存在该玩家本地。每一次发送（包括重试和再次抽取）都创建全新会话，并持续读取 SSE 到平台完成事件；明文 `ping`/`keepalive` 只视为流保活。玩家设定整理仍进行内容完整度校验；初始将领只要求得到 JSON 对象，缺项由宿主补齐，核心设定没有业务字数门槛，模型战力被忽略。初始将领先展示为本地预览，玩家可重抽或编辑，确认后才创建角色；未部署将领和私人对话不会广播到评论区。
...
固定伴生作品 `b27218e6-80f9-4c0d-91c7-4b8f87d47be8` 又在 2026-09-14 完成一次正式配置保存：

- 作品名 `猎艳疆土[b27218e680f94c0d]`，其中方括号内为从固定作品 UUID 派生的 16 位实例 ID；简介、前置词、世界观提示词和后置词均为本仓库固定模板。
- “详细介绍”当前保存 37,612 字符的 `FYOW-PROGRAM/1` 信封，程序 SHA-256 为 `9bd51e04315125e553185a58e3b64535a44524b7579feac2d6d5f23135e417db`；当前同步包含自由性癖标签、注释、偏好修改、加入流程互斥、七类将领资料、玩家/将领战力修炼、逐次模型积分记录、`fyow-host/1` 回包关联和分场景音效。
- 将领生成、玩家角色设定整理、普通将领互动、俘虏将领互动和将领记忆整理五个世界书均为 `key_region=2`、`enable=true`、`probability=100`。
- 保存接口 HTTP 200，随后导出接口 HTTP 200；上述七项逐一比对全部通过。
- 本轮新版程序保存与回读均为 HTTP 200；卡内规范化配置摘要为 `3f868e0466b4ccbe2cf9a7980741ed6d8161f3b239df4e2dd21cfc722316e8f9`；作品作者已发布新的签名控制记录并启用同一程序摘要。
- 具体操作和平台字段经验见 `docs/aiero-creation-page-save-experience.md`。

2026-09-13 又以稀疏角色输入 `茂密 / 猫亚人 / 白色头发` 执行真实模型回归：玩家上下文在新的平台会话中等到 `message_end`，四个字段长度依次为 432、177、105、147 字，全部保留输入事实并主动补全，未包含占位措辞；初始将领在另一个新会话中返回“雪铃守”，女性、战力 3260、设定 707 字，白发猫亚人、守城与长期羁绊要求均落实。两个会话编号不同，证明同一入场流程的两个输入也分别创建新会话。

将领七类资料更新后再次执行真实回归：稀疏玩家上下文四字段长度为 456、138、90、128 字；初始将领“雪狸守”返回女性、身高 158 cm、体重 47.5 kg、三围 84/57/88、基础战力 1850、外观 193 字、核心设定 591 字。两次请求均等待 `message_end` 且会话编号不同，证明新版 Schema 已在真实伴生作品生效。

2026-09-14 对全部五类任务执行真实回归：稀疏玩家上下文四字段长度为 489、188、124、155 字，初始将领“雪狸”返回女性、身高 162 cm、体重 49.5 kg、三围 84/58/88、外观 175 字、核心设定 528 字；普通互动、记忆整理和俘虏互动也都通过结构与内容校验。五次请求均收到 `message_end`，会话编号互不重复。随后以无缓存服务读取真实评论区，共定位 3 页、103 条评论，0 条不完整和无效记录，最新作者控制摘要与作品程序摘要一致，服务状态为 `ready`。

0.14.20 的两阶段入场程序与放宽后的生成世界书保存后又完成一次真实回归：初始将领“白霜”为女性、身高 162 cm、体重 51 kg、三围 86/59/89、外观 192 字、核心设定 585 字；角色整理、初始将领、普通互动、记忆整理、俘虏互动五次请求全部收到 `message_end`，五个会话编号互不重复。伴生作品回读的程序摘要为 `9544c0ec84564f56da5112733a2998ef77b2bf8356e5796e1ef2b30f655117e9`，无缓存同步读取 3 页、104 条评论，0 条不完整、0 条无效，作者控制摘要相同且状态为 `ready`。

0.14.21 已把平台积分余额和逐条模型消耗接入游戏宿主：每条消息在独立新会话完成后优先读取完成事件积分，缺省时使用调用前后平台余额差；结构校验重试同样逐条展示。固定伴生作品保存与回读均为 HTTP 200，详细介绍和最新作者控制记录的程序摘要均为 `9bd51e04315125e553185a58e3b64535a44524b7579feac2d6d5f23135e417db`。无缓存同步读取 3 页、50 条评论，在覆盖快照处停止，0 条不完整、0 条无效，状态为 `ready`。

同日用单条角色设定整理消息验证积分结算：请求在新会话 `08b70b03-1166-4b76-b592-16536221a3ee` 中收到 `message_end`，平台 `usage` 返回 `prompt_points=29`、`completion_points=39`、`total_points=68`；平台余额从 39,179,102 变为 39,179,034，差值同为 68。由此确认当前节点可以直接展示回包分项和总消耗，余额差路径仅作为兼容后备。

增量战力体系采用等级成本指数递增与阶段里程碑：单级金币费用按基础费用乘 `1.15^等级`，战力每级线性增长并在每 10 级获得 1.25 倍阶段增幅；本人和未部署自有将领共用该规则。相关公式、获得将领路径和耗时估算见 `docs/grid-conquest-companion-work.md`。
...
| 评论分页、地图增量归并、私信唤醒 | `electron/online-world-service.cjs` | 已按平台实际旧到新分页修正尾页回扫，并完成 103 条真实评论重进测试 |
| 游戏卡整包、固定作品/作者绑定与摘要校验 | `electron/online-world-card.cjs` | 已实现内置卡、导入/导出、配置快照、作者回读匹配与迁移重绑定 |
| 64×64 规则引擎 | `electron/grid-world-game.cjs` | 已实现固定格子、计时、经济、战斗、玩家/将领增量战力、将领与记忆 |
| 游戏前端 | `electron/desktop/online-world/grid-conquest/` | 已实现四问入场、ACG 词条、地图缩放/拖动、计时任务、战力修炼、行军队伍、随行/部署/俘虏将领、七类档案、互动、来信箱与作者专属服主指令 |
...
1. 第二账号从 `installed-apps/{workId}` 下载同一信封，完成“作者初始化 → 玩家加入 → 本机行动 → 领地增量 → 另一客户端回读”完整赛季。
2. 制造 100 页以上的夹具账本，测试完整快照提前终止、跨页分片、评论删除、折叠、乱序和重复事件；当前单元测试已覆盖旧到新分页、尾页增量、分片/摘要与基础提前终止，真实作品已验证 3 页/103 条，尚缺真实百页压力测试。
3. 实测个人“自定义配置”世界书的跨账号隔离以及部署移除/召回恢复；当前代码会在变化后回读确认，但仍需要真实页面的字段兼容验收。
4. 实测迁移作品的全字段兼容。当前流程会导出 JSON、创建新作品、映射并保存配置、回读核对核心字段、改名并回读旧作品，在新作品写入控制/地图快照，最后才在旧作品发布签名定向迁移；客户端看到指令后自动切换。平台导出字段与保存体字段并不完全同构，所以任一回读不一致都会停在“迁移草稿”而不发布重定向。该映射尚未在真实首张作品上完成验收，且新作品是否仍需创作页手动发布取决于平台当前可见性规则。
5. 增加本地存档导出/导入和损坏修复提示。当前私人进度只在本机保存，换机或清理应用数据前需要玩家主动备份。
6. 继续增加全作品级评论积压上限和程序 CPU/内存卡死监控。评论读取与地图增量归并已经按平台时间戳排序；定向消息有每分钟 12 次发送限制及每发送者每分钟 20 次接收限制。iframe 已断网隔离，但 JavaScript 无限循环仍需要独立可销毁执行进程才能可靠恢复。
7. 增加设备换绑/恢复流程。当前赛季会把平台账号和首次加入时的 Ed25519 设备密钥固定绑定；重装系统或丢失 Windows 加密密钥后，新密钥提交会被拒绝，需要由旧设备或作品作者签名换绑，相关界面尚未加入。
