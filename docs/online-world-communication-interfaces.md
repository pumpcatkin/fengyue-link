...
- 全量读取上限 10000 页；平时从最新页开始，在已知记录或覆盖快照处提前停止。
...
每次尝试都不携带旧会话编号；平台必须返回一个从未使用过的新 `conversationId`。工具完整读取事件流直到结束事件，之后才解析 JSON 并向游戏前端回包。单次请求最长等待 180 秒，同一工具实例的模型请求严格串行。
...
模型返回只作为结构化内容，不直接决定金币、领地、兵力、时间或战斗结果。生成失败时：初始将领会阻止入场并回滚；占地奖励将领在公开地图已经提交后进入本地指数退避队列，30 秒起步、最长 15 分钟，保留稳定发现编号避免重试生成两名将领。
