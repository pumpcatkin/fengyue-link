...
describe("Electron platform API regressions", () => {
  ...
  it("auto-selects a production node and rebases every work page to the login node", () => {
    ...
    const preload = readFileSync(new URL("../electron/preload.cjs", import.meta.url), "utf8");
    const renderer = readFileSync(new URL("../electron/desktop/renderer.js", import.meta.url), "utf8");
    expect(main).toContain("origins.filter(origin => TRUSTED_PLATFORM_ORIGINS.has(origin) && origin !== DEFAULT_ORIGIN)");
  ...
  it("revalidates the two token-relevant files on every hidden packaged startup and shows only branded result dialogs", () => {
    ...
    expect(renderer).toContain("if(security.verified&&!(!next?.loggedIn&&automaticUpdate))");
    expect(renderer).toContain("showReleaseVerificationFailure(security,update)");
    expect(renderer).toContain("const update=await api.checkForUpdates()");
    ...
    expect(html).toContain('id="official-notice-open" type="button">检查并自动更新</button>');
