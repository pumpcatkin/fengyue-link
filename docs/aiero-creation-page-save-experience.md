...
最新一次保存进一步把将领返回拆分为姓名、性别、身高、体重、三围、外观设定和核心设定，并把修炼后的战力资料加入互动上下文。后置词继续要求稀疏输入主动补全，将领生成世界书把第四问设为最高优先级绑定要求，玩家设定世界书禁止以“未提供”等词跳过创作。普通互动、俘虏互动和记忆整理不再接收账号 UUID，书信使用宿主本机映射的一次性目标键。保存后用导出接口逐项回读；随后真实执行全部五类任务，每次都新建会话并等到 `message_end`，五份结果均通过各自 Schema 校验。
...
世界书：5 条
详细介绍：34,594 字符
程序 SHA-256：0540f0932ef25e6904499566053563fb1196e14d44c61c6d2a2106209b7c1586
```
...
只做回读验证时设置 `FYOW_VERIFY_ONLY=1`；排查导出别名时设置 `FYOW_INSPECT=N`，其中 `N` 为 1～30；用 `FYOW_VERIFY_SYNC=1` 可模拟没有本地缓存的重进同步；用 `FYOW_PROBE_MODEL=1` 会依次真实调用五类模型任务并确认五个独立新会话。常规执行会改写目标作品，使用前应再次确认固定 `workId`。
