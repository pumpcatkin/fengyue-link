...
当前安装版自带 Ed25519 签名的运行证明，每次启动都会在本地验签并重新计算 2 个关键运行时文件的 SHA-256：`风月联机工具.exe` 与 `resources/app.asar`。登录不依赖 GitHub 连通性；GitHub 仅用于检查和下载更新。运行证明随安装内容分发，不保存可用于绕过校验的成功记录。
...
程序启动时的界面只显示“正在对照版本号”“版本号对照完成”或简短的版本号对照失败信息，不再向普通用户解释完整性校验、安全目的、发布渠道、公钥或密钥。登录页不提供手动验证入口。内部仍会在每次启动时完成内置运行证明、版本号和关键文件摘要校验；失败时停止登录。
...
构建安装包时先为预打包目录生成内置运行证明，再生成安装包和在线签名清单：

```powershell
$env:FENGYUE_RELEASE_SIGNING_KEY = "D:\独立的离线密钥目录\fengyue-release-ed25519-private.pem"
npm run pack:win
...
第二条命令会断开 GitHub 响应，验证每次启动仍能通过内置证明重新验签、校验 2 个关键文件并阻断篡改。第三条命令读取打包 EXE，确认 Electron 会在应用代码加载前校验 `app.asar`，且不会从目录回退或接受 Node 调试环境变量。
...
- 安装版每次启动：在后台自动验证安装包内置的签名运行证明，不等待 GitHub；界面仅显示版本号对照状态。
- 启动验证范围：只校验 2 个关键运行时文件——`风月联机工具.exe` 与 `resources/app.asar`。`app.asar` 包含主进程、preload、界面及令牌处理代码；主 EXE 是加载这些代码的 Electron 宿主。
- 启动验证成功：当前启动期间允许登录；不会写入或读取本地放行记录，也不要求预先安装游戏卡。
- 启动验证失败：停止登录并显示简短的版本号对照失败信息；下次启动会重新读取内置签名运行证明并校验文件，不沿用上次结果。
- 检测到更新：当前版本低于 GitHub 最新签名版本时停止登录，并在后台自动下载。下载后的安装包必须与 Ed25519 签名清单中的文件名、大小和 SHA-256 完全一致，验证成功后才允许静默安装；失败时可在登录前界面直接重试自动更新。
- 未登记版本：本机版本高于或不同于 GitHub 最新签名版本时停止登录，并引导用户返回唯一官方发布页。
- 同版本后续启动：重新读取并验签安装目录中的运行证明，重新计算上述 2 个文件摘要；不沿用任何本地放行结果。
