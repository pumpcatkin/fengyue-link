...
const GRID_COMPANION_COPY = Object.freeze({
  ...
  worldBook: Object.freeze([
    ...
    Object.freeze({
      ...
      key_region: 2,
      value_type: 0,
      value: "任务：在将领完成一次互动后，更新其长期记忆。必须只返回一个完整 JSON 对象。category 只能为 speech 或 deed；summary 不超过 120 字，emotion 不超过 40 字，intimacyDelta 为 -5 到 5 的整数。compactMemory 必须同时包含“言谈：”和“经历：”，总长度不超过 1000 个汉字；合并重复事件时使用年份区间和次数，保留对象设定名、事件、情绪和关系结果。历任主公、被俘、降服与易主事实必须完整保留。面向玩家的记忆文本使用设定名，不显示账号编号。\n输出 Schema：{\"category\":\"speech|deed\",\"summary\":\"事件摘要\",\"emotion\":\"情绪与关系感受\",\"intimacyDelta\":1,\"compactMemory\":\"言谈：……\\n经历：……\"}",
