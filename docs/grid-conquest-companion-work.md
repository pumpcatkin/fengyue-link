...
将 `release-cache/online-world/grid-conquest-description-envelope.txt` 的完整内容放进作品“详细介绍”。可在信封前后写普通介绍文字，但一份详细介绍只能保留一个活动信封。生成器会把 HTML、CSS 和 JavaScript 合成单文件后 gzip/Base64URL 编码；当前信封共 13,864 个字符，解码后的压缩包为 10,335 字节。
...
1. **已完成**：保存作品后，从导出接口回读名称、简介、前置词、主提示词、后置词、两个用户范围世界书和完整程序信封；HTTP 200，程序 SHA-256 为 `cb906e2fb1ca8454203dc8ffa4ed12887e20694057cb3b7e52eb10adbaa26d62`。
