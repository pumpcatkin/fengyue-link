...
if (process.type === "renderer") {
  ...
  ipcMain.handle("qa:api", (_event, name, args) => {
    ...
    if (name === "listOnlineWorldCards") return { cards: [{ cardId: "cc.aiero.fyow.grid-conquest.official", gameId: "cc.aiero.fyow.grid-conquest", title: "猎艳疆土", version: 5, workId: "b27218e6-80f9-4c0d-91c7-4b8f87d47be8", workName: "猎艳疆土[b27218e680f94c0d]" }], activeCardId: null };
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      window.webContents.send("qa:onOnlineWorldState", { status: "ready", initialized: true, revision: 1, card: { cardId: "cc.aiero.fyow.grid-conquest.official", title: "猎艳疆土" }, work: { id: "fixture", name: "猎艳疆土[b27218e680f94c0d]" }, control: { seasonId: "fixture-season" }, account: { accountId: "a", username: "本地测试" }, world: { seed: "fixture", startedAt: Date.now(), revision: 1, cells: { "4,7": { ownerAccountId: "a", soldiers: 80, generalIds: [] }, "5,7": { ownerAccountId: "b", soldiers: 60, generalIds: [] } }, players: { a: { accountId: "a", displayName: "本地测试", gold: 1000, position: { x: 4, y: 7 }, fieldArmySoldiers: 0, carriedGeneralIds: ["g1"] }, b: { accountId: "b", displayName: "北境玩家" } }, generals: { g1: { id: "g1", name: "青禾", power: 500, holderAccountId: "a", loyalToAccountId: "b", capturedFromAccountId: "b", status: "carried", setting: "善守城，重信义。", masterHistory: [{ accountId: "b", fromYear: 1, toYear: 2 }], captivityHistory: [], interactionHistory: [], memoryText: "言谈：暂无\n经历：[1年]战败被俘" } }, jobs: {} }, directInbox: [{ messageId: "dm1", fromAccountId: "b", type: "general-letter", payload: { generalName: "青禾", text: "愿暂息兵戈，共商边界。" }, createdAt: Date.now() }], mapFacts, serverNow: Date.now(), program: { source: "card-package", digest: "fixture-card-loaded" }, programHtml });
      await settle();
